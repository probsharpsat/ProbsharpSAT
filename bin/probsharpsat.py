...
def main():
    ...
    parser.add_argument("-pcc", action='store_true', help="use probablistic component caching (PCC)", dest='PCC')
    parser.add_argument("-csvsads", action='store_true', help="use CSVSADS branching heuristic", dest='CSVSADS')
    parser.add_argument("-pc", action='store_true', help="use polarity cache (PC)", dest='PC')
    parser.add_argument("-is", help="use independent support (IS)", dest='IS', action='store_true')
    parser.add_argument("-edr", help="use exponentially decaying randomness (EDR)", dest='EDR', action='store_true')
    parser.add_argument("-lso ", type=int, default=0, help="use LSO heuristic after LSO decisions", dest='LSO')
    ...
    cmd = "/usr/bin/time --verbose -o timeoutfile ./probsharpsat -cs " + str(args.cs) + " -t " + str(args.t) + " "
    ...
    print("The total user time taken by ProbsharpSAT is: ", total_user_time)
